...
state *simulator::run( state *istate, qocircuit *qoc, int method ){
    ...
    return run(istate,qoc,method,-1);
}
...
//
//----------------------------------------
p_bin *simulator::run(qodev *circuit, int method, int nthreads){
...
//  Variables
    ...
    // Run simulation
    output=run(circuit->inpt,circuit->circ, method, nthreads);
...
//
//---------------------------------------------------------------
state *simulator::run( state *istate, qocircuit *qoc, int method, int nthreads ){
...
//  Variable
    ...
    switch (method)
    {
        ...
        case 3: // GlynnR
            ...
            break;
        case 4: // OMP RyserF
            return RyserF(istate,qoc,nthreads);
            break;
        case 5: // OMP RyserR
            return RyserR(istate,qoc,nthreads);
            break;
        case 6: // OMP RyserF + Selected distribution
            return Fast_Ryser(istate,qoc,true,nthreads);
            break;
        case 7: // Fast Glynn R  + Selected distribution
            return Fast_Ryser(istate,qoc,false,nthreads);
            break;
        default:
...
state *simulator::run( state *istate, ket_list* olist, qocircuit *qoc, int method){
    ...
    return run( istate, olist, qoc, method, -1 );
}
...
//
//---------------------------------------------------------------
state *simulator::run( state *istate, ket_list* olist, qocircuit *qoc, int method, int nthreads ){
//  state     *istate;      // Input state
...
//  Variable
    ...
    switch (method)
    {
        ...
        case 2: // Glynn
            ...
            break;
        case 4: // Ryser
            return RyserS(istate,olist,qoc,nthreads);
            break;
        default:
            empty_state=new state(istate->nph, istate->nlevel,mem);
            return empty_state;
...
//--------------------------------------------------------------
//
// Permanent calculation method (using Ryser formula with parallelism)
// for a restricted list of outputs.
//
//---------------------------------------------------------------
state *simulator::RyserS( state *istate, ket_list *olist, qocircuit *qoc, int nthreads ){
//  state     *istate;         // Input state
//  ket_list  *olist;          // Output ket list
//  qocircuit *qoc             // Circuit to be simulated
//  Variables
        ...
        for(oket=0;oket<olist->nket;oket++){
            ...
            if(nph==tocc){
                if(nph>0){
                    ...
                    // Normalize
                    coef=istate->ampl[iket]*ryser_omp(Ust,nthreads)/(sqrt(t)*sqrt(s));
                }else{
                    coef=1.0;
                }
                ...
                if(abs(coef)>xcut){
                    index= ostate->add_term(coef,olist->ket[oket]);
                    if(index<0){
                        cout << "Simulator(RyserS): Warning! Simulation canceled because the memory limit has been exceeded.  Increase *mem* for more memory." << endl;
                        return ostate;
                    }
                }
...
//
//---------------------------------------------------------------
state *simulator::Fast_Ryser( state *istate, qocircuit *qoc, bool F, int nthreads){
//  state     *istate;              // Input state
//  qocircuit *qoc;                 // Circuit to be simulated
//  bool       F;                   // Full distribution or restricted to one photon by channel True: Full/False: Restricted
//  int        nthreads;            // Number of threads
//  Variables
    int        c_nph;               // Constrained number of photons
    int        eph;                 // Expected number of photons
    int        maxch;               // Larger channel number
    int        nempty;              // Number of channels with zero photons
    int        ndec;                // Number of conditions
    ...
    veci       constraint;          // Constraint vector
    mati       def;                 // Post-selection matrix definition
    state     *ostate;              // Output state. Restricted to kets that do no contribute to post-selection.
//  Hash tableS
...
//  Auxiliary index
    ...
    ostate=new state(istate->nph,istate->nlevel,mem);
    ndec=qoc->ncond;
    def=qoc->det_def;
    ...
    ch.resize(c_nph+nempty);
    pch.resize(c_nph+nempty);
    k=0;
    l=0;
    for(ich=0;ich<ndec;ich++){
    ...
    // LOOP Polarization
    pol=new int[c_nph+1]();
    constraint.resize(istate->nlevel);
    while(pol[c_nph]==0){
        ...
        while(tim[c_nph]==0){
            ...
            eph=0;
            for(l=0;l<istate->nlevel;l++) constraint(l)=-1;
            for(iph=0;iph<(c_nph+nempty);iph++){
            for(im=0;im<qoc->nm;im++){
            for(is=0;is<qoc->ns;is++){
                k=qoc->i_idx[ch(iph)][im][is];
                if((iph<c_nph)&&(im==pol[iph])&&(is==tim[iph])&&((im==pch(iph))||(pch(iph)==-1))){

                    if(constraint(k)<0) constraint(k)=0;
                    constraint(k)=constraint(k)+1;
                    eph=eph+1;
                }else{
                    constraint(k)=0;
                }
            ...
            if(eph==c_nph){
                // Check that we have not generated that constraint before.
                ...
                for(i=0;i<(ndec*qoc->nm*qoc->ns);i++){
                    keyprj[i]=constraint(i);
                }
                prjvalue=hashval(keyprj,ndec*qoc->nm*qoc->ns, c_nph);
                ...
                if(vprjhash==prjhash.end()){
                    ...
                    // Obtain the output state provide the given constraint
                    if(F) aux_RyserF(istate,ostate,qoc,c_nph,constraint,nthreads);
                    else  aux_RyserR(istate,ostate,qoc,c_nph,constraint,nthreads);
...
//--------------------------------------------------------------
//
// Permanent calculation method (using parallelized Ryser formula)
// Full distribution.
//
//---------------------------------------------------------------
state *simulator::RyserF( state *istate, qocircuit *qoc, int nthreads){
//  state     *istate;           // Input state
//  state     *ostate;           // Output state. Output variable
//  qocircuit *qoc               // Circuit to be simulated
//  int        nthreads;         // Number of threads.
//  Variables
    int        c_nph;            // Constraint photons. Photons that are part of the constraint.
    veci       constraint;       // Constraint vector.
    state     *ostate;           // Output state
//  Auxiliary index
    int        i;                // Aux index

    //Initialize variables for unconstrained run
    c_nph=-1;
    constraint.resize(qoc->nlevel);
    for(i=0;i<qoc->nlevel;i++) constraint(i)=-1;
    ostate=new state(istate->nph,istate->nlevel,mem);

    aux_RyserF( istate, ostate, qoc, c_nph, constraint, nthreads);
...
//
//---------------------------------------------------------------
state *simulator::RyserR( state *istate, qocircuit *qoc, int nthreads){
//  state     *istate;           // Input state
//  state     *ostate;           // Output state. Output variable
//  qocircuit *qoc               // Circuit to be simulated
//  int        nthreads;         // Number of threads.
...
//  Auxiliary index
    ...
    aux_RyserR( istate, ostate, qoc, c_nph, constraint, nthreads);
...
//--------------------------------------------------------------
//
// Auxiliary method for the permanent calculation (using parallelized Ryser formula)
// Full distribution.
//
//---------------------------------------------------------------
void simulator::aux_RyserF( state *istate, state* ostate, qocircuit *qoc, int c_nph, veci constraint, int nthreads){
//  state     *istate;           // Input state
//  state     *ostate;           // Output state. Output variable
//  qocircuit *qoc               // Circuit to be simulated
//  int        c_nph;            // Constraint photons. Photons that are part of the constraint.
...
//  Variables
    int    nph;                  // Number of photons present in input ket.
    int    uc_nph;               // Unconstrained number of photons. Photons that are not part of the constraint.
    int    nlevel;               // Number of levels (qoc has this information, but it is put in this variable for easy access)
    int    uc_nlevel;            // Number of levels without a constraint.
    int    index;                // Ket list position where a new term of the output state is stored.
    int   *pos;                  // Level where each photon is located. "Photon position"
    int   *occ;                  // Occupation
    cmplx  coef;                 // Coefficient for the transformation of a ket.
    cmplx  s;                    // Normalization coefficient of the input ket
    cmplx  t;                    // Normalization coefficient of the output ket
    veci uclevels;               // List of levels without a constraint
    matc   Ust;                  // Matrix to calculate the permanent
//  Index
    int    iket;                 // Index of input kets elements
    int    ilin;                 // Index of input levels
...
//  Auxiliary index
    ...
    // Process contraint information
    uc_nlevel=qoc->nlevel-qoc->ncond*qoc->nm*qoc->ns;
    if(c_nph<0){ // No constraint want to be applied
        c_nph=0;
        uc_nlevel=qoc->nlevel;
    }
    uclevels.resize(uc_nlevel);
    j=0;
    ...
    if(abs(istate->ampl[iket])>xcut){
        ...
        // Check all the possible outputs
        pos=new int[uc_nph+1]();
        occ=new int[nlevel]();
        Ust.setZero(nph,nph);

        while(pos[0] < uc_nlevel){
             // Calculate variables from the output ket
            ...
            t=1.0;
            for(j=0;j<nlevel;j++){
                if(constraint(j)>=0) occ[j]=constraint(j);
                t=t*(cmplx)factorial(occ[j]);
            }
            ...
            if(abs(coef)>xcut){
                index= ostate->add_term(coef,occ);
                if(index<0){
                    cout << "Simulator(aux_RyserF): Warning! Simulation canceled because the memory limit has been exceeded.  Increase *mem* for more memory." << endl;
                    // Free memory
                    delete[] pos;
                    delete[] occ;
                    // Finish partial calculation
                    return;
                }
            }

            // Obtain new photon level "position"
            pos[uc_nph-1] += 1; // xxxxN -> xxxxN+1
            for (i = uc_nph; i > 0; i -= 1) {
                if (pos[i] > uc_nlevel - 1) // if number spilled over: xx0(n-1)xx
                {
                    pos[i - 1] += 1; // set xx1(n-1)xx
                    for (j = i; j <= uc_nph; j += 1)
                        pos[j] = pos[j - 1]; // set xx11..1
                }
            }
        }
...
//
//---------------------------------------------------------------
void simulator::aux_RyserR( state *istate, state* ostate, qocircuit *qoc , int c_nph, veci constraint, int nthreads){
//  state     *istate;         // Input state
//  state     *ostate;         // Output state. Output variable
//  qocircuit *qoc             // Circuit to be simulated
//  int        c_nph;          // Constraint photons. Photons that are part of the constraint.
...
//  Auxiliary index
    ...
    if(abs(istate->ampl[iket])>xcut){
        ...
        //Initalize the bitmask of the occupation state. Bit of level j is 1 if j is occupied.
        bitmask.resize(0,0);
        bitmask.resize(uc_nph,1);
        bitmask.resize(uc_nlevel,0);
        ...
        do{
            ...
            if(abs(coef)>xcut){
                index= ostate->add_term(coef,occ);
                if(index<0){
                    cout << "Simulator(RyserR): Warning! Simulation canceled because the memory limit has been exceeded.  Increase *mem* for more memory." << endl;
                    // Free memory
                    delete occ;
                    // Finish partial calculation
                    return;
