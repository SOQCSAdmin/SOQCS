...
extern "C" {
    ...
    void qoc_pol_beamsplitter(long int qoc, int i_ch1, int i_ch2, int P, double theta){qocircuit* aux=(qocircuit*)qoc; aux->pol_beamsplitter(i_ch1,i_ch2, P, theta);}
    ...
    void dev_pol_beamsplitter(long int dev, int i_ch1, int i_ch2, int P, double theta){qodev* aux=(qodev*)dev; aux->pol_beamsplitter(i_ch1,i_ch2, P, theta);}
    ...
    // Run methods
    long int sim_run(long int sim,long int dev, int method, int nthreads){ simulator *auxsim=(simulator *) sim; qodev  *auxdev=(qodev *) dev; return (long int) auxsim->run(auxdev,method,nthreads);}
    long int sim_run_state(long int sim,long int st,long int qoc, int method, int nthreads ){ simulator *auxsim=(simulator *) sim; state  *auxst=(state *) st; qocircuit *auxqoc=(qocircuit*)qoc; return (long int) auxsim->run(auxst,auxqoc,method,nthreads);}
    long int sim_run_list(long int sim,long int st,long int ls,long int qoc, int method, int nthreads ){ simulator *auxsim=(simulator *) sim;
                                                                                              state  *auxst=(state *) st;
                                                                                              state  *auxls=(state *) ls;
                                                                                              qocircuit *auxqoc=(qocircuit*)qoc;
                                                                                              return (long int) auxsim->run(auxst,auxls,auxqoc,method,nthreads);
                                                                                            }
    ...
    char *sim_get_sample(long int sim,long int dev, int mode ){ simulator *auxsim=(simulator *) sim;
                                                                qodev  *auxdev=(qodev *) dev;
                                                                p_bin* auxpb=auxsim->sample(auxdev,1);
                                                                string value =auxpb->tag(0);
                                                                delete auxpb;
                                                                char* char_array=new char[value.length()+1];
    ...
    char *sim_get_metro(long int sim,long int dev, int mode){   simulator *auxsim=(simulator *) sim;
                                                                qodev  *auxdev=(qodev *) dev;
                                                                p_bin *auxpb;
                                                                double p;
                                                                tie(auxpb,p)=auxsim->metropolis(auxdev,mode,1,0,1);
                                                                string value =auxpb->tag(0);
                                                                delete auxpb;
                                                                char* char_array=new char[value.length()+1];
                                                                strcpy(char_array, value.c_str());
    ...
    //--------------------------------------------------------------------------------------------------------------------------
    // MTHREAD
    long int mt_new_mthread(int i_mem){ return (long int) new mthread(i_mem);}
    void mt_destroy_mthread(long int mt){mthread *aux=(mthread *)mt; delete aux; }
    void mt_send_work(long int mt,long int st, long int qoc, int method){ mthread *auxmt=(mthread *) mt; state  *auxst=(state *) st; qocircuit *auxqoc=(qocircuit*)qoc; auxmt->send_work(auxst,auxqoc,method);}
    long int mt_receive_work(long int mt){ mthread *auxmt=(mthread *) mt; return (long int) auxmt->receive_work();}
    //--------------------------------------------------------------------------------------------------------------------------
    ...
    double qk_retrieve_buffer(){
        return buffer;
    }

   int *qk_initgx(int *qdef, int nqbits, long int dev) {  qodev *auxdev=(qodev*)dev;
                                                          mati imat=to_mati(qdef,2,nqbits);
                                                          state* auxst=auxdev->inpt->encode(imat,auxdev->circ);
                                                          int *aux=new int[nqbits];
                                                          for(int i=0;i<nqbits;i++) aux[i]=auxst->ket[0][i];
                                                          delete auxst;
                                                          return aux;
                                                       }


    double *qk_init_mtx(long int sim, int *qdef, int nqbits, long int dev)  {simulator *auxsim=(simulator *)sim;
                                                                             qodev *auxdev=(qodev*)dev;
                                                                             mati imat=to_mati(qdef,2,nqbits);
                                                                             matc auxmtx=auxsim->init_mtx(imat,auxdev->inpt,auxdev->circ);
                                                                             double *auxdouble=to_dptr(auxmtx);
                                                                             return auxdouble;}

    double *qk_assemble(long int sim, int *in_qdef, int *out_qdef, int nqbits, long int dev, int method, int nthreads, int norm) {
                                                                             simulator *auxsim=(simulator *)sim;
                                                                             qodev *auxdev=(qodev*)dev;
                                                                             mati imat=to_mati(in_qdef,2,nqbits);
                                                                             mati omat=to_mati(out_qdef,2,nqbits);
                                                                             cmplx *auxmtx;
                                                                             tie(auxmtx,buffer)=auxsim->assemble(imat,omat,auxdev->inpt,auxdev->circ,method,nthreads,norm);
                                                                             return (double *) auxmtx;}


   int *qk_pol_initgx(int *qdef, int nqbits, long int dev) {  qodev *auxdev=(qodev*)dev;
                                                              veci ivec=to_veci(qdef,nqbits);
                                                              state* auxst=auxdev->inpt->pol_encode(ivec,auxdev->circ);
                                                              int *aux=new int[nqbits];
                                                              for(int i=0;i<nqbits;i++) aux[i]=auxst->ket[0][i];
                                                              delete auxst;
                                                              return aux;
    ...
    double *qk_pol_init_mtx(long int sim, int *qdef, int nqbits, long int dev)  {simulator *auxsim=(simulator *)sim;
                                                                                 qodev *auxdev=(qodev*)dev;
                                                                                 veci ivec=to_veci(qdef,nqbits);
                                                                                 matc auxmtx=auxsim->pol_init_mtx(ivec,auxdev->inpt,auxdev->circ);
                                                                                 double *auxdouble=to_dptr(auxmtx);
                                                                                 return auxdouble;}

    double *qk_pol_assemble(long int sim, int *in_qdef, int *out_qdef, int nqbits, long int dev, int method, int nthreads, int norm){
                                                                             simulator *auxsim=(simulator *)sim;
                                                                             qodev *auxdev=(qodev*)dev;
                                                                             veci ivec=to_veci(in_qdef,nqbits);
                                                                             veci ovec=to_veci(out_qdef,nqbits);
                                                                             cmplx *auxmtx;
                                                                             tie(auxmtx,buffer)=auxsim->pol_assemble(ivec,ovec,auxdev->inpt,auxdev->circ,method,nthreads,norm);
                                                                             return (double *) auxmtx;}
